...
const PSU = () => {
    ...
    useEffect(() => {
        fetch(
            "https://raw.githubusercontent.com/actuallynotrena/switchblade-data/master/psu.json"
        )
    ...
    _getItem = async (item) => {
        await AsyncStorage.setItem("PSU", JSON.stringify(item));
        Toast.show({
            type: "success",
    ...
    const renderItem = ({ item }) => (
        ...
        >
            <View style={{ maxWidth: "72%" }}>
                ...
                >
                    {item.power_capacity ? `${item.power_capacity}W` : ""}
                    {item.gpu_connector ? ` - ${item.gpu_connector}` : null}
                    {item.price
                        ? " - " +
                          item.price +
    ...
    if (SearchText.startsWith("capacity:")) {
        filteredData = data.filter((item) => {
            const capacity = item.power_capacity || "";
    ...
    } else if (SearchText.startsWith("gpu_pins:")) {
        filteredData = data.filter((item) => {
            const pins = item.gpu_connector || "";
            if (SearchText.split(":")[1] == "") {
                return false;
            } else {
                return pins
        ...
        return (
            <SafeAreaProvider>
                ...
                >
                    ...
                    >
                        <HeaderText text={language.select_psu} />
                        <TouchableOpacity
                            onPress={() => {
                                navigation.navigate("PSUHelper");
                            }}
                        >
                            <Text
                                style={{
                    ...
                    >
                        <TouchableOpacity
                            onPress={() => setSearchText("capacity:450")}
                        >
                            <TagComponent TagText="450W" />
                        </TouchableOpacity>
                        <TouchableOpacity
                            onPress={() => setSearchText("capacity:1000")}
                        >
                            <TagComponent TagText="1000W" />
                        </TouchableOpacity>
                        <TouchableOpacity
                            onPress={() => setSearchText("gpu_pins:6+2")}
                        >
                            <TagComponent TagText="6+2 pins" />
                        </TouchableOpacity>
                        <TouchableOpacity
